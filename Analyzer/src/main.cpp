...
int main(int argc, char* argv[]) {
	switch(argc){
		...
		case 2:
				...
				break;
		default:
				std::cout << "Usage: " << " periodic" << std::endl; 
				exit(1);
	}
  ...
  for (int i(0); i < core; ++i) {
      ...
      cv::line(image, cv::Point(LEFT_CORE_PAD, core_line*(i+1)), cv::Point(WIDTH,core_line*(i+1)), cv::Scalar(0,0,0), 3, 4);
      cv::putText(image, std::string("core" + std::to_string(i)), cv::Point(0, core_line*(i+1)), cv::FONT_HERSHEY_TRIPLEX, 0.5, cv::Scalar(0,0,0), 1, CV_AA);
  ...
  for (int i(0); i < core; ++i) {
    for (int j(0); (size_t)j < cpu_taskset.at(i).size(); ++j) {
      ...
      cv::rectangle(image, cv::Point(start_mag, core_line*(i+1) - core_line/2), cv::Point(end_mag, core_line*(i+1)), colors, -1, 8);
      cv::rectangle(image, cv::Point(start_mag, core_line*(i+1) - core_line/2), cv::Point(end_mag, core_line*(i+1)), cv::Scalar(0,0,0), 2, 8);
      cv::putText(image, std::string(std::to_string(start)), cv::Point(start_mag-5, core_line*(i+1)+15), cv::FONT_HERSHEY_TRIPLEX, 0.5, cv::Scalar(0,0,0), 1, CV_AA);
      cv::putText(image, std::string(std::to_string(end)), cv::Point(end_mag-5, core_line*(i+1)+15), cv::FONT_HERSHEY_TRIPLEX, 0.5, cv::Scalar(0,0,0), 1, CV_AA);
      cv::putText(image, std::string(g[desc[index]].name_p), cv::Point((start_mag+end_mag)/2-5, core_line*(i+1) - ((core_line/2) + 5)), cv::FONT_HERSHEY_TRIPLEX, 0.5, cv::Scalar(0,0,255), 1, CV_AA);
    }
  }
