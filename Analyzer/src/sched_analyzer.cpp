  ...
  for (int i(0); (size_t)i < Queue.size(); ++i) {	
    ...
    /* determine core that be set sched_(v_)node_t*/
      ...
      while (true) {
        ...
        get_sched_node_info_in_selected_cores(min_start_time, sched_node_info,
                                              v_can_use_core);
        int is_vacancy_node = get_sched_vacancy_node_info_in_selected_cores(
						Queue.at(i).runtime_p, min_start_time, sched_vacancy_node_info, 
						v_can_use_core);
        ...
        show_sched_cpu_tasks();
				...
				for (int node_core(0); node_core < Queue.at(i).core_p - 1; 
             ++node_core) {
          sched_node_info_t searched_sched_node_info;
          sched_v_node_info_t searched_sched_vacancy_node_info;
          get_sched_node_info_in_selected_cores(v_used_start_time.back(),
                                                searched_sched_node_info,
                                                v_can_use_core);
          int is_vacancy_node = get_sched_vacancy_node_info_in_selected_cores(
							Queue.at(i).runtime_p, v_used_start_time.back(),
              searched_sched_vacancy_node_info, v_can_use_core);

          std::cout << "start time:" << v_used_start_time.back() << std::endl;
          if (is_vacancy_node == 1 &&
              (searched_sched_vacancy_node_info.start_time <=
               searched_sched_node_info.start_time)) {
            std::cout << "vacancy sched start time:"
                      << searched_sched_vacancy_node_info.start_time
                      << std::endl;
            ...
            if (searched_sched_vacancy_node_info.start_time !=
                v_used_start_time.back()) {
              success = false;
            }
          } else {
            std::cout << "sched start time:"
                      << searched_sched_node_info.start_time << std::endl;
      ...
      show_sched_cpu_tasks();
        ...
        sched_node_info_t searched_sched_node_info;
        sched_v_node_info_t searched_sched_vacancy_node_info;
        get_sched_node_info_in_selected_cores(
            min_start_time, searched_sched_node_info, v_can_use_core);
        int is_vacancy_node = get_sched_vacancy_node_info_in_selected_cores(
						Queue.at(i).runtime_p, min_start_time,
						searched_sched_vacancy_node_info, v_can_use_core);
        if (is_vacancy_node == 1 &&
            (searched_sched_vacancy_node_info.start_time <=
             searched_sched_node_info.start_time)) {
          ...
          set_sched_vacancy_node(index, run_time,
                                 searched_sched_vacancy_node_info);
        } else {
          v_used_core.push_back(searched_sched_node_info.core_index);
          if (searched_sched_node_info.need_empty) {
            sched_node_t sched_empty_node;
            int empty_start_time = 0;
            if (v_sched_cpu_task_.at(searched_sched_node_info.core_index)
                    .empty()) {
              empty_start_time = 0;
            } else {
              empty_start_time =
                  v_sched_cpu_task_.at(searched_sched_node_info.core_index)
                      .back()
                      .esc_time;
            }
            create_empty_node(empty_start_time,
                              searched_sched_node_info.start_time,
                              sched_empty_node);
          ...
          sched_node_t sched_node;
					create_sched_node(Queue.at(i).id_p,
                            searched_sched_node_info.start_time,
                            Queue.at(i).runtime_p, sched_node);

          /* set sched_node to v_sched_cpu_task_ */
          v_sched_cpu_task_.at(searched_sched_node_info.core_index)
              .push_back(sched_node);
        }
      }
      continue;
    }
    ...
    int is_vacancy_node = get_sched_vacancy_node_info(
				Queue.at(i).runtime_p, min_start_time, sched_vacancy_node_info);      
    ...
    /* push back in v_sched_cpu_task_ */
      ...
      if (sched_node_info.need_empty) {
        ...
        if (v_sched_cpu_task_.at(sched_node_info.core_index).empty()) {
          ...
          empty_start_time =
              v_sched_cpu_task_.at(sched_node_info.core_index).back().esc_time;
        }
      ...
      sched_node_t sched_node;
  		create_sched_node(Queue.at(i).id_p, sched_node_info.start_time,
                         Queue.at(i).runtime_p, sched_node);
...
void SchedAnalyzer::get_can_use_core(const std::vector<int> &v_used_core,
                                     std::vector<int> *v_can_use) {
  v_can_use->clear();
  for (int core(0); core < get_spec_core(); ++core) {
    bool skip(false);
    for (int used_core_i(0); (size_t)used_core_i < v_used_core.size();
         ++used_core_i) {
    ...
    for (int j(0); (size_t)j < v_sched_cpu_task_.at(i).size(); ++j) {
      if (v_sched_cpu_task_.at(i).at(j).empty) {
        std::cout << "\x1b[36m(core" << i
                  << ")\x1b[m\x1b[35mName:\x1b[m\x1b[31mempty\x1b[m "
                     "\x1b[34m\t\t\tlaxity:"
                  << "NULL"
                  << "\x1b[m\t\x1b[31m["
        ...
        std::cout << "\x1b[36m(core" << i << ")\x1b[m\x1b[35mName:"
                  << g[desc[v_sched_cpu_task_.at(i).at(j).node_index]].name_p
                  << "\x1b[m\x1b[34m\t\t\tlaxity:"
                  << g[desc[v_sched_cpu_task_.at(i).at(j).node_index]].laxity_p << "\x1b[m\t["
                  << v_sched_cpu_task_.at(i).at(j).start_time << "~"
...
int SchedAnalyzer::get_cpu_taskset(
    std::vector<V_sched_node> &v_sched_cpu_task) {
...
int SchedAnalyzer::get_sched_node_info_in_selected_cores(
    const int min_start_time, sched_node_info_t &sched_node_info,
    std::vector<int> v_can_use_core) {
  /* determine core that be set sched_node_t*/
  struct score_t {
    int score;
...
int SchedAnalyzer::get_sched_vacancy_node_info_in_selected_cores(
    const int run_time, const int min_start_time,
    sched_v_node_info_t &sched_vacancy_node_info,
    std::vector<int> v_can_use_core) { /* init sched_vacancy_node_info */
  sched_vacancy_node_info.start_time = -1;
  sched_vacancy_node_info.core_index = -1;
  sched_vacancy_node_info.empty_index = -1;
  sched_vacancy_node_info.need_before_empty = false;
