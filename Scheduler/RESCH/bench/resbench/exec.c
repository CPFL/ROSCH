...
void exec_task(task_t *task, int loop_1ms, int simtime, int policy)
{
	int k; /* do not use i! */
	...
	rt_reserve_start(C, xcpu_handler);
	rt_run(timeout);
...
int schedule(FILE *fp, int m, int loop_1ms, int simtime, int policy)
{
	int i, j, ret, status;
	pid_t pid;
	...
	for (i = 0; i < nr_tasks; i++) {
		...
		if (pid == 0) { /* the child process. */
			...
			exec_task(&tasks[i], loop_1ms, simtime, policy);
		}
	}
